...
- Read `docs/ARCHITECTURE.md` for ownership and new-app conventions. Keep app code under `apps/<id>` and shared code under `packages/`. Use `server/index.js` for composition; `server.js` is only a compatibility launcher.
...
- Read `docs/FUNCTIONALITY_LEDGER.md` and `README.md` before continuing work. Update the ledger with real implementation and verification status before ending a work session.
- Keep the SQL schema in `apps/advocate/database/schema.sql` and `packages/database/schema.sql`. Runtime persistence is `data/chambers.sqlite`; do not replace this with browser-only state or JSON files.
