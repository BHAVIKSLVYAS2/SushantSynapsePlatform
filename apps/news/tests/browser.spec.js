...
test('News foundation: protected preview, planned catalogue, honest empty states, responsive themes and print',async({page})=>{
  ...
  expect(catalog.apps.find(app=>app.id==='news')).toMatchObject({status:'Planned',accessible:false});
...
test('saved editions render ten stories and satire, reopen by date, handle gaps and suppress stale responses',async({page})=>{
  ...
  await expect(page.locator('.saved-satire')).toContainText('AI-generated satire');
  ...
  await page.getByRole('button',{name:'Latest edition',exact:true}).click();await expect(page.locator('.saved-story')).toHaveCount(10);
  ...
  await page.getByRole('button',{name:'Latest edition',exact:true}).click();await expect(page.locator('.saved-story h2').first()).toContainText('2025-01-03');
