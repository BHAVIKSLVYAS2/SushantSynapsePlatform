...
**News manual release deployed (2026-09-18):** Full owner-triggered publication is live at `/news`; catalogue is Available and explicitly granted readers can open editions. Ten recent English PIB releases become credited source excerpts plus clearly labelled, locally templated fictional satire by Bhavik. Scope is India public affairs; no AI key is required and 05:00 IST automation remains deferred. GDELT is no longer the production source. SQLite snapshots, leases, attempt limits, atomic immutable publication, date archives and cache reuse are verified. Provider attempts are capped at twenty per UTC day; each PIB attempt uses at most 21 fixed-host requests with a 90-second deadline.

**Release verification:** 54 Node tests passed. The exact staged News-only checkout also passed all 28 Node tests and all three News Chrome workflows. All 15 Chrome workflows were verified: twelve existing-app workflows passed in the full run, then all three News workflows passed after fixing a mock month format and isolating their databases. Granted readers can read but cannot fetch or view operational previews. Mobile/desktop, themes, printing, archive gaps, stale response suppression, atomic failure recovery and restart cache reuse are covered. The rendered newspaper screenshot was reviewed.

**Production evidence:** Backup `data/backups/pre-news-publication-20260918-233556.sqlite` passed integrity validation before restarting the identified supervised Node process. Public home, Chambers, Fund Lens, health and News return 200; News HTML/JS/CSS hashes match the release. Anonymous News status/archive requests return 401. The real 2026-09-18 edition is published with ten stories and one satire; a second fetch returned the stored edition with no extra provider call. SQLite integrity is `ok`; only news_editions, news_stories, news_runs and news_fetch_budget changed compared with the backup. All existing account/business/document tables are unchanged. No demo data or account grants were added. Unrelated Fund Lens work remains outside this News commit.
